        ...
        public override async Task<InputFormatterResult> ReadRequestBodyAsync(InputFormatterContext context, Encoding effectiveEncoding)
        {
            ...
            using (var reader = new StreamReader(request.Body, effectiveEncoding))
            {
                try
                {
                    ...
                    var contact = new Contact() { Name = split[0].Substring(2), EmailAddress = split[1] };

                    await ReadLineAsync("FN:", reader, context);

                    var idLine = await ReadLineAsync("Id:", reader, context);
                    contact.Id = Guid.Parse(idLine); // idLine.Substring(4);
